  ...
  )
{
  ...
  do {
	...
	MicroSecondDelay(20);
	DEBUG ((DEBUG_ERROR, "Wait4Bit: Timeout! Value=%x, Mask=%x, Retry=%d\n",Value,Mask,Retry));
  } while (Retry--);
    ...
    IN DWUSB_OTGHC_DEV *DwHc)
{
  ...
  //intr_mask |= DWC2_GINTMSK_RXSTSQLVL; DMA enabled turn this off
  ...
  )
{
  ...
  DEBUG((DEBUG_INFO,"DwHcDeferredTransfer: CallBackFunction\n"));
  ...
  )
{
  ...
  if (DwHc->StatusBuffer == NULL) {
    DEBUG ((DEBUG_ERROR, "CreateDwcOtgDevice: No pages available for StatusBuffer\n"));
    return EFI_OUT_OF_RESOURCES;
  }
  ...
  Status = DmaMap (MapOperationBusMasterCommonBuffer, DwHc->AlignedBuffer, &BufferSize,
                   (PHYSICAL_ADDRESS *)&DwHc->AlignedBufferBusAddress, &DwHc->AlignedBufferMapping);
  if (EFI_ERROR (Status)) {
    DEBUG ((DEBUG_ERROR, "CreateDwcOtgDevice: DmaMap: %r\n", Status));
    return Status;
  }
