  ...
  )
{
  ...
  Status = MmcHost->SendCommand (MmcHost, MMC_CMD8, 0x1aa);
  if (Status == EFI_SUCCESS) {
  ...
  while (Timeout > 0) {
    ...
    if (Status == EFI_SUCCESS) {
      ...
      Status = MmcHost->SendCommand (MmcHost, MMC_ACMD41, 0x40200000);
      if (!EFI_ERROR (Status)) {
      ...
      Status = MmcHost->SendCommand (MmcHost, MMC_CMD1, 0x200000);
      if (!EFI_ERROR (Status)) {
        Status = MmcHost->ReceiveResponse (MmcHost, MMC_RESPONSE_TYPE_OCR, Response);
        if (EFI_ERROR (Status)) {
          DEBUG ((EFI_D_ERROR, "MmcIdentificationMode() : Failed to receive OCR, Status=%r.\n", Status));
